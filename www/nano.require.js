...
(function (nano) {
    ...
    function getCacheBustedUrl(relative) {
    ...
    // dependencies
    function deepLoad(moduleInfo, callback, parentLocation, id) {
        ...
        request.onload = function () {
            if (request.status === 200 || moduleInfo.t) {
                ...
                (moduleInfo.t = moduleInfo.t || request.response).replace(/(?:^|[^\w\$_.])require\s*\(\s*["']([^"']*)["']\s*\)/g, function (_, id) {
                    deps.push(id);
                });
                ...
                deps.map(function (dep) {
                    deepLoad(resolveModuleOrGetExports(moduleInfo.l, dep), loaded, 
                ...
                if (parentLocation) {
                    ...
                    deepLoad(moduleInfo.n = resolveModuleOrGetExports(parentLocation += "../", id), callback, parentLocation, id);
                }
                else {
     ...
     * @param base
     */
    function resolve(relative, fileExtension, base) {
        baseElement.href = base || "";
        // If the relative url doesn't begin with a ".", then it's in node_modules
        const cacheBustedUrl = getCacheBustedUrl(relative);
    ...
    function requireCss(relative, callback) {
    ...
    nano.requireCss = requireCss;
    function requireJS(id, callback) {
        deepLoad(resolveModuleOrGetExports("", id), function (err, module) {
            try {
    ...
    if (scriptElement) {
        ...
        const path = scriptElement.dataset.main;
        requireJS(path);
    }
})(nano || (nano = {}));
//# sourceMappingURL=nano.require.js.map